...
class Student():
    ...
    def write_programs(self, i = 1):
        self.num_programs = self.num_programs + i
...
def personal_map(fun_obj, lst):
    new = []
    for i in lst:
        new.append(fun_obj(i))
    return new
...
names_with_not_too_much_seniority = [p.name for p in programmers if len(p.name) > p.years_UM]
...
def main_filterer(file_names_list):
    return (len_check(readfiles(file_names_list)))
